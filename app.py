...
def reply_to(message, reply, kdb_type):
    if kdb_type != 'none' and kdb_type != 'address':
        bot.send_message(message.chat.id, reply, reply_markup=getattr(kb, kdb_type))
...
def send_welcome(message):
    bot.send_photo(message.chat.id, main_cfg.welcome_pic, "Приветствую, "+message.from_user.first_name+'! Я обучающийся бот!', reply_markup=kb.welcome_kbd)
...
def echo_all(message):
    ...
    with open(main_cfg.path, 'r', encoding='utf-8') as jfr:
	    ...
	    qx = sorted([SM(lambda x: x == ' ', q, i['q']).ratio() for i in data[0]], reverse=True)
